...
void measurement(int i_sample){
    ...
    if(stagger_factor==NULL){
        ...
        for(int k=0;k<Nz;k++){
            for(int j=0;j<Ny;j++){
                for(int i=0;i<Nx;i++){
                    stagger_factor[i+j*Nx+k*Nx*Ny] = (i+j+k)%2*2-1;
                }
            }
        }
    }
...
void set_lattice_jq3_3d_ladder_uniform(int nx, int ny, int nz, double qbond){
    Nsite = nx*ny*nz;
    Nj = 3*Nsite;
    Nq = 2*Nsite;
    ...
    for(int k=0;k<nz;k++) {
        for(int j=0;j<ny;j++) {
            for(int i=0;i<nx;i++) {
                Bond2index[i_bond*6+0] = i+nx*j+nx*ny*k;
                Bond2index[i_bond*6+1] = ((i+1)%nx)+nx*j+nx*ny*k;
    ...
    for(int k=0;k<nz;k++) {
        for(int j=0;j<ny;j++) {
            for(int i=0;i<nx;i++) {
                Bond2index[i_bond*6+0] = i+nx*j+nx*ny*k;
                Bond2index[i_bond*6+1] = i+nx*((j+1)%ny)+nx*ny*k;
                Bond2index[i_bond*6+2] = -1;
                Bond2index[i_bond*6+3] = -1;
                Bond2index[i_bond*6+4] = -1;
                Bond2index[i_bond*6+5] = -1;
    ...
    for(int k=0;k<nz;k++) {
        for(int j=0;j<ny;j++) {
            for(int i=0;i<nx;i++) {
                Bond2index[i_bond*6+0] = i+nx*j+nx*ny*k;
                Bond2index[i_bond*6+1] = i+nx*j+nx*ny*((k+1)%nz);
                Bond2index[i_bond*6+2] = -1;
                Bond2index[i_bond*6+3] = -1;
                Bond2index[i_bond*6+4] = -1;
                Bond2index[i_bond*6+5] = -1;
    ...
    for(int k=0;k<nz;k++) {
        for(int j=0;j<ny;j++) {
            for(int i=0;i<nx;i++) {
                Bond2index[i_bond*6+0] = ((i+0)%nx)+nx*((j+0)%ny)+nx*ny*k;
                Bond2index[i_bond*6+1] = ((i+1)%nx)+nx*((j+0)%ny)+nx*ny*k;
                Bond2index[i_bond*6+2] = ((i+0)%nx)+nx*((j+1)%ny)+nx*ny*k;
                Bond2index[i_bond*6+3] = ((i+1)%nx)+nx*((j+1)%ny)+nx*ny*k;
                Bond2index[i_bond*6+4] = ((i+0)%nx)+nx*((j+2)%ny)+nx*ny*k;
                Bond2index[i_bond*6+5] = ((i+1)%nx)+nx*((j+2)%ny)+nx*ny*k;
                Bondst[i_bond] = qbond;
    ...
    for(int k=0;k<nz;k++) {
        for(int j=0;j<ny;j++) {
            for(int i=0;i<nx;i++) {
                Bond2index[i_bond*6+0] = ((i+0)%nx)+nx*((j+0)%ny)+nx*ny*k;
                Bond2index[i_bond*6+1] = ((i+0)%nx)+nx*((j+1)%ny)+nx*ny*k;
                Bond2index[i_bond*6+2] = ((i+1)%nx)+nx*((j+0)%ny)+nx*ny*k;
                Bond2index[i_bond*6+3] = ((i+1)%nx)+nx*((j+1)%ny)+nx*ny*k;
                Bond2index[i_bond*6+4] = ((i+2)%nx)+nx*((j+0)%ny)+nx*ny*k;
                Bond2index[i_bond*6+5] = ((i+2)%nx)+nx*((j+1)%ny)+nx*ny*k;
                Bondst[i_bond] = qbond;
    ...
    while((c=getopt(argc,argv,"hx:y:z:q:b:n:k:t:s:f:m:l:p:"))!=-1){
        switch(c){
            case 'h':
                ...
                printf("\t\t 0 : 3d JQ3 model (ladder uniform)\n");
...
int main(int argc, char** argv){
    ...
    if(LatticeType==0) set_lattice_jq3_3d_ladder_uniform(Nx,Ny,Nz,Qbond);
